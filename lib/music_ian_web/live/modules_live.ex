defmodule MusicIanWeb.ModulesLive do
  ...
  def handle_event("start_lesson", %{"id" => lesson_id}, socket) do
    # Redirigir al Explorador con la lección como parámetro
    {:noreply, push_navigate(socket, to: ~p"/?start_lesson=#{lesson_id}")}
  end
  ...
  def render(assigns) do
    ...
    <!-- Module Progress Summary -->
                <% completed_lessons =
                  Enum.count(module_item.lessons_with_results, fn item ->
    ...
    <!-- Lesson Score -->
                      <div class="flex flex-col items-end gap-2">
                        <%= if lesson_item.latest_result do %>
                          <% total_notes =
                            lesson_item.latest_result.correct_count +
                              lesson_item.latest_result.error_count

                          accuracy =
                            if total_notes > 0,
                              do: round(lesson_item.latest_result.correct_count / total_notes * 100),
                              else: 0 %>
                            ...
                            <div class="text-xs text-slate-600 space-y-1 border-t border-slate-200 pt-2 mt-2">
                              ...
                              <div>
                                ...
                                <span class="font-semibold text-emerald-600">
                                  {lesson_item.stats.total_correct || 0}
                                </span>
                              </div>
                              <div>
                                <span class="text-slate-500">Errores:</span>
                                <span class="font-semibold text-red-600">
                                  {lesson_item.stats.total_errors || 0}
                                </span>
                              </div>
                            </div>
                          <% end %>
                          ...
                          <button
                            ...
                            phx-value-id={lesson_item.lesson.id}
                          ...
                          >
                            ...
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M5.25 5.653c0-.856.917-1.398 1.591-.784l11.54 9.257a1.647 1.647 0 010 2.554l-11.54 9.257c-.674.614-1.591.07-1.591-.784V5.653z"
                              />
                            </svg>
                            Reintentar
                          </button>
