      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div id="device-select" phx-hook="MidiDeviceSelector" class="space-y-3">
            <select id="midi-devices" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <p class="text-xs text-slate-600 mb-3">Presiona FUNCTION en el piano y luego haz clic aquí:</p>
            <div class="space-y-3">
              <button
          ...
          <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <div class="space-y-3">
              <div>
                <label class="block text-sm font-semibold text-slate-900 mb-2">Nota MIDI (0-127):</label>
                <input
                  type="number"
                  id="test-note"
                  value={@test_note}
              ...
              <div>
                <label class="block text-sm font-semibold text-slate-900 mb-2">Velocidad (0-127):</label>
                <input
                  type="number"
                  id="test-velocity"
                  value={@test_velocity}
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
          ...
          <div class="mb-6">
            ...
            <select id="beat-select" phx-change="set_beat" name="beat" class="w-full px-2 py-2 border-2 border-slate-200 rounded-lg text-sm">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
          ...
          <div class="space-y-3">
            <div class="grid grid-cols-2 lg:grid-cols-6 gap-2">
              <button onclick="window.midiDeviceSelector?.sendPC(0, 'PC 0')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 0
              </button>
              <button onclick="window.midiDeviceSelector?.sendPC(1, 'PC 1')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 1
              </button>
              <button onclick="window.midiDeviceSelector?.sendPC(5, 'PC 5')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 5
              </button>
              <button onclick="window.midiDeviceSelector?.sendPC(10, 'PC 10')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 10
              </button>
              <button onclick="window.midiDeviceSelector?.sendPC(20, 'PC 20')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 20
              </button>
              <button onclick="window.midiDeviceSelector?.sendPC(50, 'PC 50')" class="py-2 px-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded text-xs">
                PC 50
              </button>
            </div>

            <div class="border-t border-slate-200 pt-4">
              <label class="block text-sm font-semibold text-slate-900 mb-2">PC Personalizado + C5:</label>
              <div class="flex gap-2">
                <input type="number" id="pc-number" placeholder="PC #" min="0" max="127" value="0" class="flex-1 px-3 py-2 border-2 border-slate-200 rounded text-sm" />
                <button onclick="window.midiDeviceSelector?.sendPCThenNote()" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-semibold">
                  Enviar PC + C5
                </button>
              </div>
              <p class="text-xs text-slate-500 mt-2">Envía PC personalizado, espera 100ms, luego envía C5 (MIDI 72)</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
          <h2 class="text-xl font-bold text-slate-900 mb-4">5. Brute Force - Test Todos los MIDI Messages</h2>
          <p class="text-sm text-slate-600 mb-4">
          ...
          <div class="mt-4 p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded text-sm text-yellow-900">
            <strong>⚠️ Advertencia:</strong> El brute force enviará muchos mensajes MIDI. Esto puede tardar unos segundos. Mantén el log abierto para ver qué se envía.
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
          ...
          <div class="space-y-3">
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
              <button onclick="window.midiDeviceSelector?.sendCC(7, 127, 'CC 7 (Volume)')" class="py-2 px-3 bg-orange-600 hover:bg-orange-700 text-white rounded text-xs">
                CC 7 (Volume)
              </button>
              <button onclick="window.midiDeviceSelector?.sendCC(10, 64, 'CC 10 (Pan)')" class="py-2 px-3 bg-orange-600 hover:bg-orange-700 text-white rounded text-xs">
                CC 10 (Pan)
              </button>
              <button onclick="window.midiDeviceSelector?.sendCC(64, 127, 'CC 64 (Sustain)')" class="py-2 px-3 bg-orange-600 hover:bg-orange-700 text-white rounded text-xs">
                CC 64 (Sustain)
              </button>
              <button onclick="window.midiDeviceSelector?.sendCC(91, 127, 'CC 91 (Reverb)')" class="py-2 px-3 bg-orange-600 hover:bg-orange-700 text-white rounded text-xs">
                CC 91 (Reverb)
              </button>
            </div>
            ...
            <div class="border-t border-slate-200 pt-4">
              ...
              <div class="flex gap-2">
                <input type="number" id="cc-number" placeholder="CC #" min="0" max="127" value="7" class="flex-1 px-3 py-2 border-2 border-slate-200 rounded text-sm" />
                <input type="number" id="cc-value" placeholder="Value" min="0" max="127" value="127" class="flex-1 px-3 py-2 border-2 border-slate-200 rounded text-sm" />
                <button onclick="window.midiDeviceSelector?.sendCustomCC()" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-semibold">
                  Enviar
                </button>
              </div>
            </div>
        ...
        <div class="mt-8 bg-blue-50 border-l-4 border-blue-500 p-6 rounded">
          <p class="text-sm text-blue-900 leading-relaxed">
            ...
            <br />
            1. El piano debe estar conectado via USB MIDI a tu computadora
            <br />
            2. Haz clic en "Conectar a dispositivo seleccionado"
            <br />
            3. Prueba los botones para enviar MIDI Note On al piano
            <br />
            4. Abre midimonitor.com simultáneamente para ver qué MIDI se envía
            <br />
        ...
        <div class="mt-8 bg-white rounded-lg shadow-lg p-6">
          ...
          <div id="midi-log" class="bg-slate-100 p-4 rounded text-sm font-mono max-h-64 overflow-y-auto">
