defmodule MusicIanWeb.Components.Music.Controls do
  ...
  def scale_controls(assigns) do
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4 flex flex-col overflow-hidden">
      ...
      <!-- Hands Mode Toggle -->
      <div class="mb-4 px-1">
        <button phx-click="toggle_hands" class={"w-full flex items-center justify-between px-3 py-2 rounded-lg border transition-all " <> if(@hands_mode == :double, do: "bg-purple-50 border-purple-200 text-purple-700", else: "bg-slate-50 border-slate-200 text-slate-600")}>
          ...
          <div class={"w-8 h-4 rounded-full relative transition-colors " <> if(@hands_mode == :double, do: "bg-purple-500", else: "bg-slate-300")}>
            <div class={"w-3 h-3 bg-white rounded-full absolute top-0.5 transition-all " <> if(@hands_mode == :double, do: "left-4.5", else: "left-0.5")}></div>
          </div>
        </button>
      </div>
      ...
      <div class="mb-4 space-y-2 px-1">
        ...
        <div class="flex items-center justify-between bg-slate-50 rounded-lg p-1 border border-slate-200">
          <button phx-click="change_octave_right" phx-value-octave={@right_octave - 1} class="p-1 text-slate-400 hover:text-slate-600 hover:bg-white rounded shadow-sm transition-all" disabled={@right_octave <= 2}>
          ...
          <button phx-click="change_octave_right" phx-value-octave={@right_octave + 1} class="p-1 text-slate-400 hover:text-slate-600 hover:bg-white rounded shadow-sm transition-all" disabled={@right_octave >= 6}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
        ...
        <%= if @hands_mode == :double do %>
          <div class="flex items-center justify-between bg-slate-50 rounded-lg p-1 border border-slate-200">
            <button phx-click="change_octave_left" phx-value-octave={@left_octave - 1} class="p-1 text-slate-400 hover:text-slate-600 hover:bg-white rounded shadow-sm transition-all" disabled={@left_octave <= 1}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 12h-15" />
              </svg>
            </button>
            ...
            <button phx-click="change_octave_left" phx-value-octave={@left_octave + 1} class="p-1 text-slate-400 hover:text-slate-600 hover:bg-white rounded shadow-sm transition-all" disabled={@left_octave >= 5}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
              </svg>
            </button>
      ...
      <div class="mb-4 px-1">
        ...
        <form phx-change="select_scale" class="relative">
          <select name="type" class="w-full text-sm border border-slate-200 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-100 bg-slate-50 py-2.5 pl-3 pr-10 appearance-none cursor-pointer hover:bg-slate-100 transition-colors shadow-sm text-slate-700 font-medium">
