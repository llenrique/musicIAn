defmodule MusicIanWeb.Components.Music.Keyboard do
  @moduledoc "Componente Phoenix para el teclado virtual interactivo."
  ...
  # Base MIDI note for keyboard display (first C to show). Default C3=48.
  # In lesson mode this gets adjusted to center the target note.
  attr :keyboard_base, :integer, default: 48

  def virtual_keyboard(assigns) do
    # Calculate dynamic keyboard label
    base = assigns.keyboard_base
    start_octave = div(base, 12) - 1
    end_octave = start_octave + 2

    active_set = MapSet.new(assigns.active_notes)

    assigns =
      assigns
      |> assign(:keyboard_label, "C#{start_octave} - C#{end_octave}")
    ...
    <div class="w-full bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      <div class="bg-slate-50 px-4 py-2 border-b border-slate-200 flex justify-between items-center shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button
            phx-click="shift_keyboard"
            phx-value-direction="down"
            title="Octava abajo"
            class="w-6 h-6 flex items-center justify-center rounded bg-slate-200 hover:bg-slate-300 text-slate-600 font-bold text-sm transition-colors disabled:opacity-30"
          >
            ◀
          </button>
          <span class="text-xs font-semibold text-slate-600 min-w-[60px] text-center">
            {@keyboard_label}
          </span>
          <button
            phx-click="shift_keyboard"
      ...
      <div class="relative flex-grow select-none w-full flex justify-center items-center overflow-hidden bg-slate-100">
        <div
          class="relative h-40"
          style="width: 1008px; transform: scale(0.9); transform-origin: center center;"
        >
          <% # Pre-calculate target notes and finger for the current lesson step
          current_step =
            if @lesson_active and (@lesson_phase == :active or @lesson_phase == :demo) and
                 @current_step_index < length(@current_lesson.steps),
               do: Enum.at(@current_lesson.steps, @current_step_index),
               else: nil
          ...
          <%= for octave <- 0..2 do %>
            <%= for i <- 0..11 do %>
              <% midi = @keyboard_base + octave * 12 + i
    ...
    <!-- White Keys -->
              <%= if !is_black do %>
                <div
                  ...
                  cond do
                    is_target -> "bg-blue-200 hover:bg-blue-300"
                    is_active -> if(is_root, do: "bg-purple-100 hover:bg-purple-200", else: "bg-emerald-50 hover:bg-emerald-100")
                ...
                >
                  ...
                  if(is_active, do: "text-slate-500", else: "text-slate-300")
                }>
              ...
              <%= if is_black do %>
                <div
                  ...
                  cond do
                    ...
                    is_active -> if(is_root, do: "bg-purple-700 hover:bg-purple-600", else: "bg-slate-600 hover:bg-slate-500")
